...
# Research Review via External Reviewer Backend (max reasoning)
...
- REVIEWER_MODEL = `gpt-6-astra` — Default model for the Codex backend, reasoning effort `max` (deep-audit tier). Must be an OpenAI model (e.g., `gpt-6-astra`, `gpt-5.5`, `o3`). Manual backend uses a model the user chooses — it must be a recognized model from a different family (OpenAI, Anthropic, Google, DeepSeek, Moonshot/Kimi, Qwen).
...
Send a detailed prompt with max reasoning, using the selected backend. For
...
mcp__codex__codex-reply:
  ...
  # replies inherit the thread's model/effort (gpt-6-astra max)
  prompt: |
