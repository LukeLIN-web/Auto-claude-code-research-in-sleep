...
- Read `../shared-references/manuscript-craft.md` before drafting Method or Experiments, and during the final pass. It is the SSOT for section policy (road-map paragraph, citation-free Method, a single Experiments section, a dedicated Advantages paragraph), float placement, appendix pointers, and LaTeX mechanics (`~\cite`, `\citet`, `$\times$`, no raster includes, no draft comments left in the source).
