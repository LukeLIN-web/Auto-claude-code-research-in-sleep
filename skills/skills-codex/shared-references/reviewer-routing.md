...
- reasoning effort: **two tiers** (since 2026-07-10; `max` needs codex-cli ≥ 0.144.1) —
  **deep-audit** skills use `max` (`proof-checker`, `kill-argument` core threads, `research-review`,
  ...
  follow-up `send_input` cannot change model/effort, and per-item `max` multiplies cost)
- round 1: `spawn_agent`
...
spawn_agent:
  model: gpt-6-astra
  reasoning_effort: xhigh   # deep-audit skills: max (see tier table above)
  message: |
    [role + task]
...
- default reviewer remains Codex at the call's declared tier (deep-audit: max / regular: xhigh) if no reviewer is specified
