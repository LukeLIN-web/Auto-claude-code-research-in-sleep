...
description: "What a paper figure is allowed to say and how it has to look in print — one figure states one conclusion, every quantity a reader compares gets its own visual channel (never a downgrade to an annotation), and every printed glyph lands in 7.5–11 pt at the width it is actually included at. Carries this repo's frozen figure rulings (caption below the graphic, no numbers and no cross-float pointers in captions, n unmarked when the denominator is full, numbers only from an SSOT, the WYSIWYG width law, the palette encodes the shape of the comparison so an ordinal ladder takes one ramp rather than opposed hues, only the mark carries hue, the settled fig:loc_conversion design and its rejected alternatives, fig:architecture is a hand-written SVG under a content lock). Use when the user says 这个图怎么画, 换个画法, 改一下颜色, 配色, 颜色太刺眼, 红蓝打架, 图太挤, 字太小, caption 写什么, 加一个面板, 'redesign this figure', 'recolour this', 'the labels collide', 'can we plot X instead'. Not for running the build: commands, paths and font-measurement live in paper/CLAUDE.md and the paper-figure-and-table-build memory."
...
| **The palette encodes the shape of the comparison, not the number of series.** Conditions that differ by *how much of the method is in play* are an ordinal ladder: one ramp that sheds chroma as it sheds machinery and ends on `C_OURS` (`C_RAMP` in `paper_plot_style.py`, `#d1d5db` → `#64748b` → `C_OURS`). Two hues (`C_BASE` against `C_OURS`) are for two genuinely independent arms; a diverging pair is for a signed quantity. `fig:locablation`'s three rungs were grey / saturated red / saturated blue — red beside blue reads as an alarm-coloured opposition, and it cut one ladder into three unrelated parties with no visual answer to *which one is ours*. | 2026-09-01 | Desaturating the baselines **is** the emphasis; nothing gets added to the panel to make the last rung read as the method. A ramp step must stay dark enough to carry white in-bar text — `#8e9aa8` lost to `#64748b` on that alone (2.9:1 against 4.3:1 on white). |
| **Only the mark carries hue.** Value labels take `INK_VAL` (`#333333`), emphasised on our own condition with **bold** and never with a second colour — a per-hue darkened text colour (`TXT = {C_BASE: "#b13a39", …}`) turns a row of 7.8 pt numbers into coloured speckle. A nested part-of-total segment is its own hue lightened, `blend(hue, 0.55)`, not `hatch="///"`: a fill pattern reads as a third series and fights every label laid near it. Marks take an `edge(hue)` hairline at 0.5 pt and `BAR_GAP` of clearance, so touching bars separate without a new colour. | 2026-09-01 | `blend()`, `edge()` and `BAR_GAP` live in `paper_plot_style.py`, so a tint, an edge and a gap are derived from the palette entry and cannot drift from it. White numbers **inside** a saturated bar stay: that is contrast, not hue. |
...
3. **Read the palette off the comparison, not off the series count.** Ask what
   separates the conditions: more machinery (ladder → one ramp), different
   parties (two hues), a sign (diverging pair). Then check nothing but the marks
   is coloured.
4. **Then fix legibility**, in this order: increase the panel, shorten the
   labels, rotate or stagger ticks, drop a redundant series — **not** reduce the
   font size.
5. **Render at print size and look**: `pdftoppm -scale-to-x $(printed_inches ×
