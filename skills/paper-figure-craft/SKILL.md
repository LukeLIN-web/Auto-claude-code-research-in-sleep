...
| **Only the mark carries hue.** Value labels take `INK_VAL` (`#333333`), emphasised on our own condition with **bold** and never with a second colour — a per-hue darkened text colour (`TXT = {C_BASE: "#b13a39", …}`) turns a row of 7.8 pt numbers into coloured speckle. A nested part-of-total segment is its own hue lightened, `blend(hue, 0.55)`, not `hatch="///"`: a fill pattern reads as a third series and fights every label laid near it. Marks take a `blend(hue, 0.24, INK1)` hairline and ~0.16 bar-widths of clearance, so touching bars separate without a new colour. | 2026-09-01 | `blend()` lives in `paper_plot_style.py`, so a tint or an edge is derived from the palette entry and cannot drift from it. White numbers **inside** a saturated bar stay: that is contrast, not hue. |
