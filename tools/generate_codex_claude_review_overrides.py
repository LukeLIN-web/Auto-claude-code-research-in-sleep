...
def normalize_description(text: str) -> str:
    ...
    text = text.replace("(Codex GPT-6-Astra max)", "(Claude via claude-review MCP)")
    ...
    text = text.replace("via GPT-6-Astra max review", "via Claude review through claude-review MCP")
...
def transform_body(text: str) -> str:
    ...
    text = text.replace("GPT-6-Astra max", "Claude review")
    ...
    text = text.replace("- ALWAYS use `model: gpt-6-astra` + `reasoning_effort: max` for reviews (deep-audit tier; capability fallback per `reviewer-routing.md`, never below `xhigh`)", "- **Always ask the Claude reviewer for strict, high-rigor feedback** in every review round.")
    ...
    text = text.replace("via a Claude reviewer via `claude-review` MCP (max reasoning)", "via `claude-review` MCP (high-rigor review)")
