...
def test_legacy_chat_keeps_retry_and_fallback_behavior():
    ...
    # The primary model gets MAX_ATTEMPTS_PER_MODEL (default 3) tries with
    # backoff before the fallback model is attempted.
    timeout_1 = MagicMock(status_code=504)
    timeout_2 = MagicMock(status_code=504)
    timeout_3 = MagicMock(status_code=504)
    success = mock_http_response("Fallback chat", "gemini-2.5-pro")
    client = mock_client_with(timeout_1, timeout_2, timeout_3, success)
