"""Load the REAL mcp-servers/minimax-chat/server.py for tests.
...
truth. Patch targets like ``tests._minimax_helpers.MINIMAX_API_KEY`` resolve
to the real server module via the sys.modules replacement below.
"""
...
_SERVER_PATH = Path(__file__).resolve().parent.parent / "mcp-servers" / "minimax-chat" / "server.py"
